...
v5 CHANGES vs v3:
  ✅ L0: H4 EMA50 kept for macro trend (solid anchor)
  ✅ L1: LOOSENED — price just needs to be above H1 EMA21 (was: full EMA21>EMA50 stack)
         → more trades pass L1 without sacrificing trend quality
  ✅ Windows: 15:00–19:00 SGT (London) + 20:00–00:00 SGT (NY)
  ✅ SL: 13 pip | TP: 26 pip | 2:1 R:R
  ✅ No trade limit (WIN-STOP removed)
  ✅ L2 state memory preserved (breakout → pullback → entry logic intact)
  ✅ All logging preserved for Railway diagnostics
"""
...
class SignalEngine:
    ...
    def _scalp_eurusd(self, instrument, state=None):
        ...
        # ── Check if L2 already fired — waiting for L3 ───────────────────
        ...
        # Kept strict: H4 EMA50 is the primary trend anchor.
        # Flip takes days — filters out choppy/ranging markets effectively.
        ...
        # ── VETO: FLAT RANGE BLOCK — H1 ATR < 4 pips ────────────────────
        ...
        # ── L1: H1 TREND ALIGNMENT — v5 LOOSENED ────────────────────────
        # v3: required full stack (price > EMA21 > EMA50)
        # v5: just needs price above H1 EMA21 (direction aligned)
        #     → allows more entries during pullbacks while still in trend
        h1_ema21 = self._ema(h1_c, 21)[-1]
        h1_close = h1_c[-1]

        bull_h1 = h1_close > h1_ema21
        bear_h1 = h1_close < h1_ema21

        if direction == "BUY" and bull_h1:
            reasons.append("✅ L1 H1 price above EMA21=" + str(round(h1_ema21, 5)) + " (v5 loosened)")
            score = 2
        elif direction == "SELL" and bear_h1:
            reasons.append("✅ L1 H1 price below EMA21=" + str(round(h1_ema21, 5)) + " (v5 loosened)")
            score = 2
        else:
            msg = ("L1 FAIL — H1 EMA21 not aligned: price=" + str(round(h1_close, 5)) +
        ...
        # L2 PASSED → save to state, wait for L3 next scan
        if state is not None:
            state["l2_pending"] = {
    ...
    def _check_l3_only(self, instrument, direction, score_so_far, reasons, state=None):
        ...
        ema_tol         = 0.00020  # 2 pip tolerance
